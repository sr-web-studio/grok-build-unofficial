...
export function activate(context: vscode.ExtensionContext): void {
  ...
  const log = (line: string) => channel.appendLine(`${timestamp()} ${redact(line)}`);

  const cwd =
    vscode.workspace.workspaceFolders?.[0]?.uri.fsPath ?? process.env.USERPROFILE ?? process.cwd();
  ...
  provider = new ChatViewProvider(context.extensionUri, session, () => channel.show(true));
  ...
  context.subscriptions.push(
    ...
    vscode.commands.registerCommand('grokBuild.showLog', () => channel.show(true)),

    vscode.commands.registerCommand('grokBuild.addSelectionToChat', async () => {
      ...
      await provider?.insertText(`\`${rel}${lines}\`\n\`\`\`${lang}\n${text}\n\`\`\`\n`);
    }),
  );
