...
import type { CreateTerminalParams, TerminalExitStatus, TerminalOutputResponse } from '../acp/types';
...
export class TerminalBridge {
  ...
  output(terminalId: string): TerminalOutputResponse {
    ...
    return { output: t.buffer, truncated: t.truncated, exitStatus: t.exitStatus ?? null };
  }

  async waitForExit(terminalId: string): Promise<TerminalExitStatus> {
    const t = this.require(terminalId);
    if (t.exitStatus) return t.exitStatus;
    return new Promise<TerminalExitStatus>((resolve) => t.exitWaiters.push(resolve));
  }
  ...
  private require(terminalId: string): TerminalRecord {
    const t = this.terminals.get(terminalId);
    if (!t) throw Object.assign(new Error(`unknown terminalId ${terminalId}`), { rpc: { code: -32602, message: `unknown terminalId ${terminalId}` } });
    return t;
  }
  ...
  private append(t: TerminalRecord, chunk: string): void {
    ...
    if (!t.released) this.opts.onOutput?.(t.id, chunk);
  }
